...
def short_response_from_tool(
    *,
    ...
    if tool_name == "get_district_optimization_problem_statement":
        ...
        return " ".join(parts)
    if tool_name == "get_district_optimization_constraints":
        text = str(tool_output.get("constraints_text", "")).strip()
        return text or "Ограничения оптимизации подготовлены."
    ...
    *,
    ...
    if tool_name == "get_district_optimization_constraints":
        return {
            ...
            "site_area": tool_output.get("site_area"),
            "source": tool_output.get("source"),
            "constraint_names": tool_output.get("constraint_names"),
            "constraints": tool_output.get("constraints"),
            "symbolic": tool_output.get("symbolic"),
        }
    return tool_output
